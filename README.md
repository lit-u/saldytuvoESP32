...
9. **Kamera testuota TIK kompiliavimu, NE fizine plokšte.** `initCamera()` (`main.cpp`) paima vieną bandomąjį kadrą per `esp_camera_fb_get()` ir loguoja jo dydį — tai bus pirmas realus patvirtinimas, kad OV5640/FPC fiziškai veikia, kai plokštė bus rankose. PWDN valdomas per CH32V003 EXIO3 (`IO_EXTENSION_CAM_PWDN_PIN`), SCCB (I2C) dalinasi bendra magistrale su touch/EXIO (`sccb_i2c_port=0`, ne atskiri pin'ai) — abu patikrinta pagal oficialų Waveshare BSP `BSP_CAMERA_DEFAULT_CONFIG` makrosą, bet nepatvirtinta veikimu.
